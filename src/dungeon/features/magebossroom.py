...
class MageBossRoom(SpecialRoom):
  ...
  def on_focus(room, game):
    if not super().on_focus(game) or "minxia" in game.parent.store.story:
      return False
    room.mage = Mage(
      faction="ally",
      facing=(0, -1)
  ...
  def on_enter(room, game):
    if not super().on_enter(game) or "minxia" in game.parent.store.story:
      return False
    room.lock(game)
  ...
  def on_complete(room, game):
    game.parent.store.story.append("minxia")
    game.anims.append([PauseAnim(
      duration=30,
