...
class Stage:
  ...
  TILE_ORDER = [FLOOR, WALL, PIT, STAIRS_DOWN, STAIRS_UP, STAIRS_EXIT, HALLWAY, FLOOR_ELEV, STAIRS, OASIS, OASIS_STAIRS]
  ...
  def is_cell_empty(stage, cell):
    ...
    if not target_tile or target_tile.solid:
      return False
    target_elem = next((e for e in stage.get_elems_at(cell) if e.solid), None)
    if target_elem:
      return False
    return True
